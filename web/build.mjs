...
let installPrefix = process.env.WASM_INSTALL_PREFIX ?? path.join(depsDir, 'install')
const buildDir = process.env.WASM_BUILD_DIR ?? path.join(projectDir, 'build-wasm')
...
function ensureEmscripten() {
  ...
  if (!fs.existsSync(envSh)) {
    console.error('Emscripten not found. Install the emsdk and source emsdk_env.sh.')
    process.exit(1)
  }
...
async function buildGmp() {
  ...
  await downloadAndExtract(`https://gmplib.org/download/gmp/${archive}`, archive, sourceDir)
